...
import * as restaurantUser from "@/lib/restaurantUser";
...
export const RestaurantUserContainer = () => {
  ...
  const deleteUser = async (id) => {
    ...
    const data = await restaurantUser.Delete(id)
  ...
  const idBodyTemplate = ({ id }) => {
    return (
      <Link href={`/dashboard/users/restaurant/${id}`} className='text-lg text-purple-400 hover:text-purple-800 transition-all'>#{id}</Link>
    )
  };
  ...
  const createUser = async () => {
    ...
    const data = await restaurantUser.Create(name, surname, email, phone, password, isActive, restaurantId, profile, lang )
    if (data.status === 'success') {
      toast.success(data.message)
      setUsers([
        ...users,
  ...
  const updateUser = async () => {
    ...
    const data = await restaurantUser.Update( id, name, surname, email, phone, password, isActive, restaurantId, profile, lang )

    if (data.status === 'success') {
      toast.success(data.message)
      restaurantUser.GetAll(true, page, rows, search).then(data => {
        setUsers(data.data)
  ...
  useEffect(() => {
    ...
    restaurant.GetAll(false).then(data => {
      setRestaurants(data.data.map(x => ({
        value: x.id,
        label: x.name
      })))
    });
              ...
              >
                <div className='w-11/12 md:w-3/4 lg:w-1/2 h-screen overflow-y-scroll absolute top-0 right-0 bg-white dark:bg-slate-900 p-5 drop-shadow-2xl transition-all'>
                  <div className='w-full flex items-center justify-between'>
                    <h1 className='text-lg text-black'>Add Restaurant User</h1>
